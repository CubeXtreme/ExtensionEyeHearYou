...
var p1 = new Promise(
    function(resolve, reject) {
        chrome.storage.sync.get('container', function(result) {
            flagVoz = result.container.flag2;
        });
        window.setTimeout(
            function() {
                console.log('Dentro funcion 250ms');
                resolve()
            }, 250);
    });
p1.then(
...
function banderaVoice() {
    ...
    if (flagVoz == true) {
        ...
        setInterval(testSpeech, 10000);
    } else {
        console.log("Desactivado Comando Voz");
    }
}